...
// import { RuntimeGlobals, __from_binding_runtime_globals, __to_binding_runtime_globals } from './RuntimeGlobals'
// import { JavascriptModulesPlugin, JsLoaderRspackPlugin } from './builtin-plugin'
...
// import type { ContextModuleFactoryAfterResolveResult, ResolveData } from './Module'
...
//       thisCompilation: new liteTapable.SyncHook<[Compilation, CompilationParams]>(['compilation', 'params']),
//       compilation: new liteTapable.SyncHook<[Compilation, CompilationParams]>(['compilation', 'params']),
//       invalid: new liteTapable.SyncHook(['filename', 'changeTime']),
//       compile: new liteTapable.SyncHook(['params']),
//       infrastructureLog: new liteTapable.SyncBailHook(['origin', 'type', 'args']),
//       failed: new liteTapable.SyncHook(['error']),
//       shutdown: new liteTapable.AsyncSeriesHook([]),
//       normalModuleFactory: new liteTapable.SyncHook<NormalModuleFactory>(['normalModuleFactory']),
//       contextModuleFactory: new liteTapable.SyncHook<ContextModuleFactory>(['contextModuleFactory']),
...
//   //   return new CacheFacade(this.cache, `${this.compilerPath}${name}`, this.options.output.hashFunction as string)
//   // }
...
//   //         }
//   //       } else {
//   //         if (this.hooks.infrastructureLog.call(normalizedName, type, args) === undefined) {
...
//   runAsChild(callback: (err?: null | Error, entries?: Chunk[], compilation?: Compilation) => any) {
//     const finalCallback = (err: Error | null, entries?: Chunk[], compilation?: Compilation) => {
//       try {
...
//    */
//   #getInstance(callback: (error: Error | null, instance?: binding.Rspack) => void): void {
...
//         (queried) =>
//           async ({ filename, targetPath, outputPath }: binding.JsAssetEmittedArgs) => {
...
//         (queried) =>
//           ({ chunk, runtimeRequirements }: binding.JsAdditionalTreeRuntimeRequirementsArg) => {
...
//             queried.call(module, Chunk.__from_binding(chunk, this.#compilation!))
...
//         (queired) => (m: binding.JsModule) => queired.call(Module.__from_binding(m, this.#compilation)),
//       ),
...
//         (queried) =>
//           ({ entry, id, codegenResults, runtimeModules }: binding.JsExecuteModuleArg) => {
...
//             ] = {})
//             const interceptModuleExecution: ((execOptions: any) => void)[] = (__webpack_require__[
//               RuntimeGlobals.interceptModuleExecution.replace(`${RuntimeGlobals.require}.`, '')
//             ] = [])
...
//         (queried) => async () => await queried.promise(this.#compilation!.chunks, this.#compilation!.modules),
//       ),
...
//             queried.call(Chunk.__from_binding(chunk, this.#compilation!), filename),
//       ),
...
//         (queried) => async (args: binding.JsNormalModuleFactoryCreateModuleArgs) => {
...
//     for (const { getHook, getHookMap, registerKind } of Object.values(this.#registers!)) {
